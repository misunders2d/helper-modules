...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.extract_button = ctk.CTkButton(
            self, text="1. Extract audio", command=self.run_extraction
        )
        self.extract_button.pack(pady=10)
        self.split_button = ctk.CTkButton(
            self, text="2. Split audio", command=self.split_audio_on_silence
        )
        self.split_button.pack(pady=10)
        self.transcribe_button = ctk.CTkButton(
            self, text="3. Transcribe audio", command=self.create_transcription
        )
        self.transcribe_button.pack(pady=10)
        self.summarize_button = ctk.CTkButton(
            self, text="4. Summarize", command=lambda: self.summarize_text(text)
        )
    ...
    def extract_audio_from_video(self):
        ...
        video_file = ctk.filedialog.askopenfile(
    ...
    def split_audio_on_silence(
        self,
        file_path=os.path.join(parent_path, "audio.mp3"),
        max_size_mb=20,
        silence_thresh=-40,
    ):
        ...
        def estimate_mp3_duration(size_mb, bitrate_kbps=192):
            ...
            return (size_mb * 1024 * 1024 * 8) / (
                bitrate_kbps * 1024
            )  # Convert MB to bits and divide by bitrate
            ...
            if chunk.dBFS < silence_thresh:
                ...
                if len(current_chunk) > 0:
                    ...
                    if len(current_chunk) / 1000 >= target_duration:
                        ...
                        current_chunk = AudioSegment.silent(
                            duration=0
                        )  # Reset current chunk
            else:
    ...
    def create_transcription(self):
        ...
        def convert_seconds(seconds):
            ...
            return (
                ...
                str(int(remaining_seconds)).zfill(2),
            )
        ...
        folder = ctk.filedialog.askdirectory(
            title="select folder with audio files", initialdir=out_path
        )
        if not folder:
            return
        ...
        try:
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You help summarize and structure large texts and meeting transcriptions",
                    },
